...
shp <- readOGR("C:/Users/14842/Documents/Projects/Exports/condec.shp")

type <- unique(shp$Deciduous)
type_df <- data.frame(ID = 1:length(type), type = type)
type_df <- type_df$ID ### 1 is coniferous, 2 is deciduous


# Place IDs
#type$ID <- type_df$ID[match(shp$Deciduous, type_df$Deciduous)] #Skipping because it didnt work

# Define RasterLayer object
r.raster <- raster() ### Blank raster
values(r.raster)

# Define raster extent to the extent of the shapefile
extent(r.raster) <- extent(shp)

# Define pixel size
res(r.raster) <- 0.1

# rasterize
ras <- rasterize(x = shp, y = r.raster)

writeRaster(ras, filename = "ras.tiff", options = c('TFW = YES'))
# ratify raster
r <- ratify(ras)
...
#shp.r <- rasterize(shp[,"Deciduous"], HIIcrop2)
#shp.r <- raster(shp.r)
#writeRaster(shp.r, filename = "shp.r.tif", options=c('TFW=YES'))
